...
class Section(TreeNode):
    ...
    def draw(self, layout, top, level=0):
        ...
        layout._drQr.draw_hbar(layout.section_fg,
...
class TreeTab(SingleWindow):
    ...
    def draw_panel(self):
        if not self._panel:
            return
        self._drQr.clear(self.bg_color)
        self._tree.draw(self, 0)
        self._drQr.draw(0, self.panel_width)
    ...
    def _create_drQr(self):
        self._drQr = drQr.DrQr(self.group.qtile, self._panel.window.wid,
            self.panel_width, self.group.screen.dheight)
        self._drQr.clear(self.bg_color)
        self._layout = self._drQr.textlayout("", "ffffff", self.font,
